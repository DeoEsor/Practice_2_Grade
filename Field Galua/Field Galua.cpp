...
public:
    ...
    byte8 galoisMul(byte8 a, byte8 b) {
        ...
        mul ^= multiply(basePolynome, mul >> 48 << 16, 0b100000000); // Вычитаем произведение Q(x) и старших блоков имеющегося произведения (сдвинутых на 4 блока), оставляя ненулевыми не более 12 младших блоков
        mul ^= multiply(basePolynome, mul >> 32,0b100000000); // Таким же образом получаем многочлен не более 8 степени, сохраняя остаток от деления. После этой операции именно остаток будет записан в mul.
        return mul;
    }
